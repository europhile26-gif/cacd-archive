...
async function hearingsRoutes(fastify, options) {
  ...
  fastify.get('/hearings', {
    schema: {
      ...
      querystring: {
        ...
        properties: {
          ...
          sortBy: { type: 'string', enum: ['hearing_datetime', 'case_number', 'created_at'], default: 'hearing_datetime' },
    ...
    if (search) {
      sql += ' AND MATCH(case_details, hearing_type, additional_information, judge, venue) AGAINST(? IN NATURAL LANGUAGE MODE)';
      params.push(search);
    }
